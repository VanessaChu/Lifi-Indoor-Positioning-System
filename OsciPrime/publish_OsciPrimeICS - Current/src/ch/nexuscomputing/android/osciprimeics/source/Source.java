...
public interface Source {
	float getVoltageDivsion(int channel, int setting);
	float getDefaultAttenuation(int channel, int setting);
	void attenuationChanged(OsciPrimeApplication app);
	void resample(boolean retrigger);
	int getResolutionInBits();
	void forceStop();
	void resample(boolean retrigger, int index);
}
